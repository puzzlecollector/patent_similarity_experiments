...
try:
    with (data_path / "test_triplet.csv").open("r", encoding="utf8") as f:
        for i, line in tqdm(enumerate(f), total=total_len, desc="calculating mrr..."):
            ...
            if rank <= 5:
                instances.append((i, q, rank, [id2fn[indices[1]],id2fn[indices[2]],id2fn[indices[3]],id2fn[indices[4]],id2fn[indices[5]]])) # row index in test.csv, query, rank, top 5 candidates
