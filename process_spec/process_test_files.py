...
test_files = os.listdir("../storage/test_spec")

def process_test_text(idx):
    with open("../storage/test_spec/" + str(test_files[idx]), "r") as f:
        data = f.read()
    triplet = data.split('\n\n\n')
    q, p, n = triplet
    try:
        ...
        processed_file = '\n\n\n'.join([q,p,n])
        with open("../storage/test_spec/" + str(test_files[idx]), "w") as f:
...
if __name__ == "__main__":
    process_map(process_test_text, range(0, len(test_files)))
