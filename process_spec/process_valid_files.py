...
valid_files = os.listdir("../storage/valid_spec")

def process_valid_text(idx):
    with open("../storage/valid_spec/" + str(valid_files[idx]), "r") as f:
        data = f.read()
    triplet = data.split('\n\n\n')
    q, p, n = triplet
    try:
        ...
        processed_file = '\n\n\n'.join([q,p,n])
        with open("../storage/valid_spec/" + str(valid_files[idx]), "w") as f:
...
if __name__ == "__main__":
    process_map(process_valid_text, range(0, len(valid_files)))
