...
class custom_collate(object):
    ...
    def __call__(self, batch):
        n = len(batch) 
        qb_input_ids, qb_attn_masks = torch.zeros((n, 10, 128),dtype=int), torch.zeros((n, 10, 128),dtype=int)
        pb_input_ids, pb_attn_masks = torch.zeros((n, 10, 128),dtype=int), torch.zeros((n, 10, 128),dtype=int)
        nb_input_ids, nb_attn_masks = torch.zeros((n, 10, 128),dtype=int), torch.zeros((n, 10, 128),dtype=int)
...
validation_dataloader = DataLoader(valid_set, batch_size=12, num_workers=4, collate_fn=collate, shuffle=True) 
