...
class custom_collate(object):
    def __init__(self):
        self.tokenizer = AutoTokenizer.from_pretrained("anferico/bert-for-patents")
        self.seq_len = 10
...
class Hi_DeBERTa_Ranker(pl.LightningModule):
   ''' no checkpoint initialization as we are not using our pre-trained deberta ''' 
    def __init__(self, train_dataloader_length, hparams=dict()):
        ...
        self.chunk_LM = AutoModel.from_pretrained("anferico/bert-for-patents")
        ...
        self.pos_encoder = PositionalEncoding(d_model=1024)
        encoder_layers = nn.TransformerEncoderLayer(d_model=1024, nhead=8)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=6)
        self.attentive_pooling = AttentivePooling(1024)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        x = torch.permute(x, (1,0,2)) # shape: [1, 10, 1024]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--setting", "-s", type=str, default="../storage/patent_experiments/default_patent_bert.yaml",
    ...
    ckpt_callback = pl.callbacks.ModelCheckpoint(
        monitor="val_loss",
        dirpath="../storage/checkpoints/patentbert64_ckpts",
