...
'''
For slightly improving the DeBERTa performanc than simply using a single CLS head
'''
...
class WeightedLayerPooling(nn.Module):
    def __init__(self, num_hidden_layers, layer_start):
...
M = 10, K = 128 tokens
using specs (title + abstract + claims) data for fine-tuning
'''
class Hi_DeBERTa_Ranker(pl.LightningModule):
    def __init__(self, train_dataloader_length, hparams=dict()):
        ...
        self.config = AutoConfig.from_pretrained("tanapatentlm/patentdeberta_base_total_1024_pwi")
        ...
        self.pooler = WeightedLayerPooling(num_hidden_layers=self.config.num_hidden_layers, layer_start=6)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        x = self.chunk_LM(input_ids=input_ids[:][0], attention_mask=attention_mask[:][0], output_hidden_states=True) 
        x = self.pooler(x.hidden_states)
        x = self.mean_pooler(x, attention_mask[:][0])
        x = torch.unsqueeze(x,dim=1)
        x = torch.permute(x, (1,0,2))

        for i in range(1, len(input_ids[:])):
            output_vec = self.chunk_LM(input_ids=input_ids[:][i], attention_mask=attention_mask[:][i], output_hidden_states=True)
            output_vec = self.pooler(output_vec.hidden_states)
            output_vec = self.mean_pooler(output_vec, attention_mask[:][i])
...
if __name__ == "__main__":
    ...
    val_set = TripletData("../storage/valid_spec")
    ...
    ckpt_callback = pl.callbacks.ModelCheckpoint(
        ...
        filename="deberta_spec_v3-{epoch:02d}-{train_loss:.2f}-{val_loss:.2f}",
        save_top_k=3,
        mode="min",
    )
