...
# we are not training, so we do not need the train_dataloader length 
checkpoint = "../storage/checkpoints/deberta_base_spec_parade_2gpu/deberta_parade_checkpoints-epoch=00-train_loss=0.00-val_loss=0.02.ckpt"
model = Hi_DeBERTa_Ranker(train_dataloader_length=None).load_from_checkpoint(checkpoint)
model.eval() 
model.freeze() 


hparams = "../storage/patent_experiments/default.yaml" 
hparams = addict.Addict(dict(load_hparams_from_yaml(hparams)))


# create and save embeddings 
dest_dir = "../storage/deberta_embeddings" 
...
q_v, p_v, n_v = [], [], []
for q, p, n in vectors:
    q_v.append(q.detach().cpu().numpy().copy())
    p_v.append(p.detach().cpu().numpy().copy())
    n_v.append(n.detach().cpu().numpy().copy())
...
with open('../storage/deberta_embeddings/epoch0_embedding.pkl','wb') as f:
