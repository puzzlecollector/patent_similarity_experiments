...
for i in tqdm(range(len(train_labels))):
    if train_labels[i] not in ipc_to_id.keys(): 
        ipc_to_id[train_labels[i]] = id_num 
        id_num += 1 
        
        
for i in tqdm(range(len(valid_labels))): 
    if valid_labels[i] not in ipc_to_id.keys(): 
        ipc_to_id[train_labels[i]] = id_num 
        id_num += 1 
...
class IPCDataset(Dataset):
    def __init__(self, arr, ipc_to_id): 
        ...
        for idx, text_path in enumerate(tqdm(text_paths)):
            try: 
                data = [] 
                data.append(f"../storage/FGH_spec_ind_claim_triplet_v1.4.1s/{text_path}") 
                data.append(ipc_to_id[labels[idx]]) 
...
class custom_collate(object):  
    def __init__(self, plm="tanapatentlm/patentdeberta_large_spec_128_pwi", id_dict = ipc_to_id): 
...
class Classifier(pl.LightningModule): 
    def __init__(self, hparams=dict(), plm="tanapatentlm/patentdeberta_large_spec_128_pwi", num_classes=50): 
