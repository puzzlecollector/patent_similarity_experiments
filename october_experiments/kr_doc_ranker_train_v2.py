...
if __name__ == "__main__": 
    ...
    ckpt_callback_train = pl.callbacks.ModelCheckpoint(
        monitor="train_loss", 
        dirpath = "kr_chkpt/", 
        filename = "KR_Ranker_epoch_end_checkoints-{epoch:02d}-{train_loss:.8f}",
        save_top_k=3, 
        mode="min" 
    ) 
    ...
    ckpt_callback = pl.callbacks.ModelCheckpoint(
        monitor="val_loss", 
        dirpath="kr_chkpt/", 
        filename="KR_Ranker_epoch_end_checkpoints-{epoch:02d}-{val_loss:.8f}", 
        save_top_k=3, 
        mode="min", 
        save_last=True
    ) 
