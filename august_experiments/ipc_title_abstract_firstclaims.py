...
class custom_collate(object):
    ...
    def __call__(self, batch):
        ...
        for idx, txt_file in enumerate(batch):
            ...
            q_text_input = q_ipc + " " + q_ttl + self.tokenizer.sep_token + q_abst + self.tokenizer.sep_token + selected_q_clm
            ...
            p_text_input = p_ipc + " " + p_ttl + self.tokenizer.sep_token + p_abst + self.tokenizer.sep_token + selected_p_clm 
            ...
            n_text_input = n_ipc + " " + n_ttl + self.tokenizer.sep_token + n_abst + self.tokenizer.sep_token + selected_n_clm 
...
train_dataloader = DataLoader(train_set, batch_size=16, num_workers=4, collate_fn=collate, shuffle=True)
validation_dataloader = DataLoader(valid_set, batch_size=16, num_workers=4, collate_fn=collate_validation, shuffle=False) 
...
for epoch_i in tqdm(range(epochs), desc="Epochs", position=0, leave=True, total=epochs):
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        for step, batch in enumerate(tepoch):
            if step%3000==0 and step > 0:
                ...
                torch.save(model.state_dict(), "../storage/ipc_title_abstract_firstclaims_epoch_{}_steps_{}_val_loss_{}.pt".format(epoch_i+1, step, avg_val_loss))
    ...
    print("saving checkpoint")  
    torch.save(model.state_dict(), "../storage/ipc_title_abstract_firstclaims_epoch_{}_train_loss_{}_val_loss_{}.pt".format(epoch_i+1, avg_train_loss, avg_val_loss)) 
