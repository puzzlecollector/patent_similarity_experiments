...
class custom_collate_use_metric_learning(object):
    ...
    def __call__(self, batch):
        ...
        for idx, txt_file in enumerate(batch):
            ...
            first_q_text = ""
            first_q_splitted = first_q.split(';') 
            for i in range(len(first_q_splitted)):
                if i == len(first_q_splitted)-1:
                    first_q_text += first_q_splitted[i] 
                else:
                    first_q_text += first_q_splitted[i] + self.tokenizer.sep_token 
                    
                        
            first_p_text = ""
            first_p_splitted = first_p.split(';') 
            for i in range(len(first_p_splitted)):
                if i == len(first_p_splitted)-1:
                    first_p_text += first_p_splitted[i] 
                else:
                    first_p_text += first_p_splitted[i] + self.tokenizer.sep_token 

            first_n_text = ""
            first_n_splitted = first_n.split(';') 
            for i in range(len(first_n_splitted)):
                if i == len(first_n_splitted)-1:
                    first_n_text += first_n_splitted[i] 
                else:
                    first_n_text += first_n_splitted[i] + self.tokenizer.sep_token 
            
            encoded_q = self.tokenizer(first_q_text, return_tensors='pt', max_length=self.chunk_size, padding='max_length', truncation=True) 
            encoded_p = self.tokenizer(first_p_text, return_tensors='pt', max_length=self.chunk_size, padding='max_length', truncation=True)
            encoded_n = self.tokenizer(first_n_text, return_tensors='pt', max_length=self.chunk_size, padding='max_length', truncation=True)
...
train_dataloader = DataLoader(train_set, batch_size=39, num_workers=4, collate_fn=collate_metric_learning, shuffle=True)
validation_dataloader = DataLoader(valid_set, batch_size=39, num_workers=4, collate_fn=collate, shuffle=False) 
...
class DeBERTa_Ranker(torch.nn.Module):
    def __init__(self, continual_learning=False, dropout_prob=0.1, multi_dropout_prob=0.2): 
        ...
        self.d_target = 256 # try dimension reduction of embeddings 
        ...
        self.fc = nn.Linear(self.d_model, self.d_target) 
        self.multi_dropout = MultiSampleDropout(multi_dropout_prob, num_samples=8, classifier=self.fc) 
...
for epoch_i in tqdm(range(epochs), desc="Epochs", position=0, leave=True, total=epochs):
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        for step, batch in enumerate(tepoch):
            if step%1000==0 and step > 0:
                ...
                torch.save(model.state_dict(), "../storage/DEBERTA_Ranker_v4_FURTHER_intermediate_checkpoint_epoch_{}_steps_{}.pt".format(epoch_i+1, step)) 
    ...
    torch.save(model.state_dict(), "../storage/DEBERTA_Ranker_v4_FURTHER_epoch_{}_train_loss_{}_val_loss_{}.pt".format(epoch_i+1, avg_train_loss, avg_val_loss))  
