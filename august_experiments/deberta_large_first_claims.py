...
train_dataloader = DataLoader(train_set, batch_size=24, num_workers=4, collate_fn=collate, shuffle=True)
validation_dataloader = DataLoader(valid_set, batch_size=24, num_workers=4, collate_fn=collate_validation, shuffle=False) 

class DeBERTa_Large_Ranker(torch.nn.Module):
    def __init__(self): 
        super(DeBERTa_Large_Ranker, self).__init__() 
        self.d_model = 1024 
        self.device = torch.device('cuda') 
        self.tokenizer = AutoTokenizer.from_pretrained("tanapatentlm/patentdeberta_large_spec_128_pwi")
        self.model = AutoModel.from_pretrained("tanapatentlm/patentdeberta_large_spec_128_pwi")
...
for epoch_i in tqdm(range(epochs), desc="Epochs", position=0, leave=True, total=epochs):
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        for step, batch in enumerate(tepoch):
            if step%3000==0 and step > 0:
                print("validating intermediate checkpoint") 
                val_loss = 0.0 
                model.eval() 
                for val_step, val_batch in tqdm(enumerate(validation_dataloader), desc="Validating", position=0, leave=True, total=len(validation_dataloader)):
                    ...
                    eval_loss = loss_func_torch(q_emb, p_emb, n_emb) 
                    val_loss += eval_loss.item() 
                avg_val_loss = val_loss / len(validation_dataloader) 
                print("average validation loss: {}".format(avg_val_loss)) 
                val_losses.append(avg_val_loss) 
                print("saving checkpoint")  
                torch.save(model.state_dict(), "../storage/DeBERTa_Large_epoch_{}_steps_{}_val_loss_{}.pt".format(epoch_i+1, step, avg_val_loss))
    ...
    print("saving checkpoint")  
    torch.save(model.state_dict(), "../storage/DeBERTa_Large_epoch_{}_train_loss_{}_val_loss_{}.pt".format(epoch_i+1, avg_train_loss, avg_val_loss)) 
