...
train_dataloader = DataLoader(train_set, batch_size=23, collate_fn=collate, shuffle=True)
validation_dataloader = DataLoader(valid_set, batch_size=23, collate_fn=collate_validation, shuffle=False) 
...
for epoch_i in tqdm(range(epochs), desc="Epochs", position=0, leave=True, total=epochs):
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        for step, batch in enumerate(tepoch):
            if step%10000==0 and step > 0:
                ...
                torch.save(model.state_dict(), "../storage/LARGE_ipc_title_firstclaims_epoch_{}_steps_{}_val_loss_{}.pt".format(epoch_i+1, step, avg_val_loss))
    ...
    print("saving checkpoint")  
    torch.save(model.state_dict(), "../storage/LARGE_ipc_title_firstclaims_epoch_{}_train_loss_{}_val_loss_{}.pt".format(epoch_i+1, avg_train_loss, avg_val_loss)) 
